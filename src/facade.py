...
class Facade:
    ...
    def encrypt_rot13(self, text: str) -> None:
        self.buffer.add_data(
            Data(text=Rot13.encrypt(text), rot_type="rot13", status="encrypt")
        )

    def decrypt_rot13(self, text: str) -> None:
        self.buffer.add_data(
            Data(text=Rot13.decrypt(text), rot_type="rot13", status="decrypt")
        )

    def encrypt_rot47(self, text: str) -> None:
        self.buffer.add_data(
            Data(text=Rot47.encrypt(text), rot_type="rot47", status="encrypt")
        )

    def decrypt_rot47(self, text: str) -> None:
        self.buffer.add_data(
            Data(text=Rot47.decrypt(text), rot_type="rot47", status="decrypt")
        )
    ...
    def load_from_file(self, file_name: str) -> None:
        ...
        for item in data:
            self.buffer.add_data(
                Data(
                    text=item["text"], rot_type=item["rot_type"], status=item["status"]
                )
            )
